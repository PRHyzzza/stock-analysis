...
/// 问财选股数据获取
/// 向 iwencai.com 发起 POST 请求，传入选股语句，返回结构化数据
pub async fn fetch_iwencai_data(query: &str, page: i32, perpage: i32) -> Result<Vec<std::collections::HashMap<String, serde_json::Value>>, String> {
    let url = "https://www.iwencai.com/gateway/urp/v7/landing/getDataList";
    ...
    // 使用 Vec<(&str, String)> + form() 让 reqwest 处理正确的 URL-encoded 编码
    let mut form: Vec<(&str, String)> = Vec::new();
    form.push(("query", query.to_string()));
    form.push(("urp_sort_way", "desc".to_string()));
    form.push(("urp_sort_index", "涨跌幅:前复权[20260623]".to_string()));
    form.push(("page", page.to_string()));
    form.push(("perpage", perpage.to_string()));
    form.push(("ret", "json_all".to_string()));
    form.push(("source", "Ths_iwencai_Xuangu".to_string()));
    form.push(("query_type", "stock".to_string()));
    form.push(("logid", "c9f6dad813ccdee7689f557b58671ffd".to_string()));
    form.push(("sessionid", "b02d9684a43410a105f84e274942ce5e".to_string()));
    form.push(("urp_use_sort", "1".to_string()));
    form.push(("user_id", "Ths_iwencai_Xuangu_x12oi86kbgo677ilkzjj3ya6simnsa6y".to_string()));
    form.push(("comp_id", "6933312".to_string()));
    form.push(("business_cat", "soniu".to_string()));
    form.push(("uuid", "24087".to_string()));
    form.push(("iwc_token", "0af3007917821795516384076".to_string()));
    form.push(("date_range[0]", "20260623".to_string()));
    form.push(("date_range[1]", "20260623".to_string()));
    ...
    let resp = client
        ...
        .send()
        .await
        .map_err(|e| format!("请求问财 API 失败: {}", e))?;

    let json: serde_json::Value = resp
        .json()
        .await
        .map_err(|e| format!("解析问财 JSON 失败: {}", e))?;

    // 检查业务状态码
    let status_code = json["status_code"].as_str().unwrap_or("-1");
    if status_code != "0" {
        let msg = json["status_msg"].as_str().unwrap_or("未知错误");
        return Err(format!("问财接口返回错误: {} (code={})", msg, status_code));
    }


    // 提取 datas 数组
    let datas = json["answer"]["components"][0]["data"]["datas"]
        .as_array()
        .ok_or_else(|| "未找到选股数据 (datas 为空)".to_string())?;

    let items: Vec<std::collections::HashMap<String, serde_json::Value>> = datas
        .iter()
        .filter_map(|d| {
            d.as_object().map(|obj| {
                obj.iter()
