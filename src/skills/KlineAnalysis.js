...
你可以通过 \`get_stock_kline\` 获取个股的 K 线数据，支持日K、周K、月K 三个周期（默认返回近 120 根，已截取最近 60 根用于分析）。

基于 K 线数据你可以自行计算和判断：
- **移动平均线（MA）**：计算 MA5、MA10、MA20、MA60，判断多头/空头排列
- **MACD**：计算 DIF、DEA、MACD 柱，判断金叉/死叉、顶背离/底背离
- **KDJ**：计算 K、D、J 值，判断超买/超卖
- **RSI**：计算 RSI6、RSI12、RSI24
- **布林带（BOLL）**：计算中轨、上轨、下轨
- **成交量分析**：量价配合、放量/缩量
- **K 线形态**：大阳线/大阴线、十字星、锤子线、吞没形态等
- **支撑/阻力位**：通过前高前低判断

当用户问"技术面""走势""K线""买点""卖点"时，调用此工具获取数据后给出计算分析。`,
};
